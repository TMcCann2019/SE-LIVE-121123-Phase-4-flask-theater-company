...
app.config['SQLALCHEMY_TRACK_MODIFICATIONS'] = False #dont absolutely need this
...
def production(title):
    ...
    if not production:
        return make_response("Production not found", 404) #simple error handling
    prod_response = {
            "title":production.title,
            "genre":production.genre,
            "director": production.director
        }
    response = make_response(jsonify(prod_response), 200)
    return response
...
if __name__ == '__main__':
    app.run(port=5555, debug=True) #the debug=True flag makes it that there is a hot reload when running the file